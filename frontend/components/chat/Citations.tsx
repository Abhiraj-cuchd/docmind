    ...
    <div className="mt-3 pt-3 border-t border-border/50">
      ...
      <div className="flex flex-wrap gap-2">
        {sources.map((s, i) => (
          <div
            key={i}
            className="flex items-center gap-1.5 bg-muted/50 border border-border/50 rounded-lg px-2.5 py-1.5 text-xs text-muted-foreground hover:text-foreground transition-colors"
          >
            <FileText className="size-3 shrink-0 text-primary/70" />
            <span className="truncate max-w-[160px]">{s.filename}</span>
            <span className="text-muted-foreground/60 shrink-0">p.{s.page}</span>
          </div>
        ))}
      </div>
    </div>
